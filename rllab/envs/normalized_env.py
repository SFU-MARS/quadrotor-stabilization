...
class NormalizedEnv(ProxyEnv, Serializable):
    ...
    def step(self, action):
        # print("")
        # print("normed action space:", self.action_space.bounds)
        # print("wrapped env action space:", self._wrapped_env.action_space.bounds)
        # print("is instance?", isinstance(self._wrapped_env.action_space, Box))
       
        # action is clipped here for specific env based on self._wrapped_env.action_space
        # not always in [-1,1] as documentation.
        if isinstance(self._wrapped_env.action_space, Box):
            # rescale the action
