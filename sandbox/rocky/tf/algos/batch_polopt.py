...
class BatchPolopt(RLAlgorithm):
    ...
    def __init__(
        ...
        # added by xlv for storing multiple evaluation paths. Format: [[(),(),()],[(),(),()],...]
        self.eval_paths = []

    def get_eval_paths(self):
        print("test get eval paths method")
        return self.eval_paths
        ...
        with tf.Session() as sess:
            ...
            for itr in range(self.start_itr, self.n_itr):
                ...
                with logger.prefix('itr #%d | ' % itr):
                    ...
                    if self.plot:
                        # modified by xlv for testing visualizing sims
                        # self.update_plot()
                        tmp_eval_path = utils.rollout(self.env, self.policy, animated=True, max_path_length=self.max_path_length,save_video=False)['observations']
                        logger.log("Saving visualization of eval paths")
                        import matplotlib.pyplot as plt
                        plt.clf()
                        # plt.hold(True)
                        plt.plot(tmp_eval_path[:,0], tmp_eval_path[:,1], '-r', linewidth=2)
                        plt.xlim([-1,1])
                        plt.ylim([-1,1])
                        plt.legend(['path'])
                        plt.savefig('/local-scratch/xlv/maml_rl/maml_examples/eval_path_' + str(itr) + '.png')
