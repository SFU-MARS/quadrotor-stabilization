...
double sweepingLF(double**** phi, int N1, int N2, int N3, int N4, double***** xs, double* dx,\
        ...
        double L, double Ixx, double grav)
{
    int i,j,k,h;
    int s1,s2,s3,s4;
    double p, q, r, t;
...
//             c = (sigma1[i][j][k]/dx[0] + sigma2[i][j][k]/dx[1] + sigma3/dx[2]);
            ...
            if (k == 0)
            {
                p = (phi[i+1][j][k][h] - phi[i-1][j][k][h])/(2*dx[0]);  // dVy
                q = (phi[i][j+1][k][h] - phi[i][j-1][k][h])/(2*dx[1]);  // dVz
                r = (phi[i][j][k+1][h] - phi[i][j][N3-1][h])/(2*dx[2]); // dPhi
                t = (phi[i][j][k][h+1] - phi[i][j][k][h-1])/(2*dx[3]);  // dWp
                
                if (- p * sin(xs[i][j][k][h][2])/m + q * cos(xs[i][j][k][h][2])/m - t * L / Ixx >= 0)
                {
                    T1 = T1Min;
                }
                else
                ...
                sigma4 =  abs(-rotDrag*xs[i][j][k][h][3]/Ixx - L*T1/Ixx + L*T2/Ixx);
                ...
                H = (-1) * ((-transDrag*xs[i][j][k][h][0]/m - T1*sin(xs[i][j][k][h][2])/m - T2*sin(xs[i][j][k][h][2])/m) * p \
                        ...
                        + (xs[i][j][k][h][3]) * r\
                        + (-rotDrag*xs[i][j][k][h][3]/Ixx - L*T1/Ixx + L*T2/Ixx) * t + 1);
